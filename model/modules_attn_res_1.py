    ...
    def forward(
        ...
        if duration_target is not None:
            ...
            duration_rounded = torch.clamp(
                (torch.round(torch.exp(log_duration_prediction.mean(0)) - 1) * d_control),
                min=0,
            )
...
class LengthRegulator(nn.Module):
    ...
    def LR(self, x, duration, max_len):
        ...
        for i,batch_t in enumerate(x):
            ...
            #batch(122,256) expand_target(122)
                expanded = torch.repeat_interleave(batch,duration[j].int(),dim=0)
                out.append(expanded)
            #expanded(864,256)
    ...
    def expand(self, batch, predicted):
        ...
        for j, batch_t in enumerate(batch):
            out.append(torch.repeat_interleave(batch_t,predicted,dim=0))
